...
public class TestcaseGenerator {
    ...
    public TestcaseGenerator(Map<String, Set<ICCMsg>> act2ReceivedICCMap) {
        ...
        this.appId = 0;
        this.apkTC = ConstantUtils.RESULTFOLDER + ConstantUtils.TESTCASEFOLDER + getAppName() + File.separator+ "testCase.iccmsg";
    ...
    public void generateTestApp(){
        ...
        //创建安卓项目
        createAndroidProject(ConstantUtils.packageName,pro_path,pro_name);
        //安卓项目的java文件所在目录，也就是src/qiu/com/fsck/k9/
        ConstantUtils.SRCFOLDER = "src" + File.separator + ConstantUtils.APKFLAG.replace(".",File.separator) + ConstantUtils.packageName.replace(".", File.separator) + File.separator;
...
//		IOUtils.delFolder(ConstantUtils.RESULTFOLDER + ConstantUtils.TESTCASEFOLDER + appModel.appName + File.separator+ apkProjectFolder );
    ...
    public void moveGeratedAPK( String projectPath) {
        ...
        String oriPath = projectPath + File.separator + "bin" + File.separator + AppModel.v().appName
                + "-debug.apk";
...
//                }
//            }
            if(line.contains("android:versionName=\"1.0\">")) line += "<uses-sdk android:targetSdkVersion=\"23\" android:minSdkVersion=\"17\"/>";
...
//                content = "<uses-sdk android:targetSdkVersion=\"23\" android:minSdkVersion=\"16\"/>\n" + content;
    ...
    public void generateManifest() {
        String pro_name = getAppName();
        String folder = ConstantUtils.RESULTFOLDER + ConstantUtils.TESTCASEFOLDER + getAppName() + File.separator;
        String projectPath =folder + ConstantUtils.GENERATEDAPP1 + pro_name ;
    ...
    public void gen_java_file(String java_file_path, ICCMsg icc, String clsname, String acdt) {
        ...
        for (String line : lines) {
            ...
            if (line.equals("{")){
                ...
                content += "\t\tComponentName cn=new ComponentName(\"" + ConstantUtils.packageName + "\",\"" + clsname.replace("_dollar_", "$")
                        + "\");\n";
...
//        refined = .replace("\"", "");
    ...
    private void copySerFiles(String pro_path) {
        ...
        IOUtils.write_to_file(desF, "package " + ConstantUtils.APKFLAG + ConstantUtils.packageName + ";\n", false);
